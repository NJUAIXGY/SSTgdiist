#!/usr/bin/env python3
"""Validate M73 memory arbitration v4 contract."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not fair_p.exists():
        return _fail(2, f"[m73][P0] missing --fair summary: {fair_p}")
    if not unfair_p.exists():
        return _fail(2, f"[m73][P0] missing --unfair summary: {unfair_p}")

    fair = _load_tensor(fair_p)
    unfair = _load_tensor(unfair_p)
    if fair is None or unfair is None:
        return _fail(2, "[m73][P0] invalid summary schema: missing tensor object")
    ...
    if fair_sched != "frfcfs" or unfair_sched != "fifo":
        return _fail(
            3,
            "[m73][P1] expected sched policy fair=frfcfs and unfair=fifo "
    ...
    if fair_qdepth <= unfair_qdepth:
        return _fail(
            3,
            "[m73][P1] expected fair bank_queue_depth > unfair "
    ...
    if fair_read_samples <= 0 or unfair_read_samples <= 0:
        return _fail(
            3,
            "[m73][P1] expected read latency samples > 0 "
    ...
    if unfair_lat <= fair_lat:
        return _fail(3, f"[m73][P1] expected unfair avg_read_latency > fair (fair={fair_lat:.3f}, unfair={unfair_lat:.3f})")
    ...
    if unfair_bus_wait <= fair_bus_wait:
        return _fail(
            3,
            "[m73][P1] expected unfair cmd_bus_wait > fair "
    ...
    if unfair_conflict <= fair_conflict:
        return _fail(
            3,
            "[m73][P1] expected unfair row_conflict > fair "
    ...
    if unfair_refresh <= fair_refresh:
        return _fail(
            3,
            "[m73][P1] expected unfair refresh_block_cycles > fair "
    ...
    if fair_mac > 0.0 and unfair_mac > 0.0 and unfair_mac > fair_mac:
        return _fail(
            3,
            "[m73][P1] expected unfair effective_mac_per_cycle <= fair "
            f"(fair={fair_mac:.6f}, unfair={unfair_mac:.6f})",
        )

    prefix = f"[m73:{args.label}] " if args.label else "[m73] "
    print(
