#!/usr/bin/env python3
"""Validate M77 scale-out v2 contract."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not single_p.exists():
        return _fail(2, f"[m77][P0] missing --single summary: {single_p}")
    if not multi_p.exists():
        return _fail(2, f"[m77][P0] missing --multi summary: {multi_p}")
    ...
    if single is None or multi is None or s_ready is None or m_ready is None:
        return _fail(2, "[m77][P0] invalid summary schema")
    if m_mesh <= s_mesh:
        return _fail(3, f"[m77][P1] expected multi mesh_size > single (single={s_mesh}, multi={m_mesh})")
    ...
    if m_coll <= s_coll:
        return _fail(3, f"[m77][P1] expected multi collective_bytes_sent > single (single={s_coll}, multi={m_coll})")
    if m_pkt <= s_pkt:
        return _fail(3, f"[m77][P1] expected multi pkt_bytes_sent > single (single={s_pkt}, multi={m_pkt})")
    ...
    if m_scale < s_scale:
        return _fail(3, f"[m77][P1] expected multi scalability score >= single (single={s_scale:.3f}, multi={m_scale:.3f})")
    ...
    if m_noc_stall <= s_noc_stall:
        return _fail(3, f"[m77][P1] expected multi noc_stall_proxy > single (single={s_noc_stall}, multi={m_noc_stall})")

    prefix = f"[m77:{args.label}] " if args.label else "[m77] "
    print(
