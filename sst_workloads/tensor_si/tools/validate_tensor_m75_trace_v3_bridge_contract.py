#!/usr/bin/env python3
"""Validate M75 trace-v3 compiler bridge contract."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    for p, name in (
        ...
        if not p.exists():
            return _fail(2, f"[m75][P0] missing --{name}: {p}")

    b_sum = _load_tensor(b_sum_p)
    d_sum = _load_tensor(d_sum_p)
    if b_sum is None or d_sum is None:
        return _fail(2, "[m75][P0] invalid summary schema: missing tensor object")

    b_params = _load_params(b_spec_p)
    d_params = _load_params(d_spec_p)

    b_ops_cfg = _to_int(b_params.get("tensor_trace_v3_op_count"))
    d_ops_cfg = _to_int(d_params.get("tensor_trace_v3_op_count"))
    b_dep_edges = _to_int(b_params.get("tensor_trace_v3_dependency_edges"))
    d_dep_edges = _to_int(d_params.get("tensor_trace_v3_dependency_edges"))
    b_windows = _to_int(b_params.get("tensor_trace_v3_tile_windows"))
    d_windows = _to_int(d_params.get("tensor_trace_v3_tile_windows"))
    b_barriers = _to_int(b_params.get("tensor_trace_v3_barrier_count"))
    d_barriers = _to_int(d_params.get("tensor_trace_v3_barrier_count"))
    b_res_windows = _to_int(b_params.get("tensor_trace_v3_resource_windows"))
    d_res_windows = _to_int(d_params.get("tensor_trace_v3_resource_windows"))
    if min(b_ops_cfg, d_ops_cfg) <= 0:
        return _fail(3, f"[m75][P1] expected trace_v3 op_count > 0 (baseline={b_ops_cfg}, dependency={d_ops_cfg})")
    if d_ops_cfg <= b_ops_cfg:
        return _fail(3, f"[m75][P1] expected dependency op_count > baseline (baseline={b_ops_cfg}, dependency={d_ops_cfg})")
    if d_dep_edges <= b_dep_edges:
        return _fail(3, f"[m75][P1] expected dependency dep_edges > baseline (baseline={b_dep_edges}, dependency={d_dep_edges})")
    if d_windows <= b_windows or d_barriers <= b_barriers or d_res_windows <= b_res_windows:
        return _fail(
            3,
            "[m75][P1] expected dependency metadata windows/barriers/resource_windows > baseline "
    ...
    if b_prog_ops <= 0 or d_prog_ops <= 0:
        return _fail(3, f"[m75][P1] expected program_ops_total > 0 (baseline={b_prog_ops}, dependency={d_prog_ops})")
    if d_mem <= b_mem:
        return _fail(3, f"[m75][P1] expected dependency mem_bytes_read_total > baseline (baseline={b_mem}, dependency={d_mem})")
    if d_coll <= b_coll:
        return _fail(3, f"[m75][P1] expected dependency collective_bytes_sent > baseline (baseline={b_coll}, dependency={d_coll})")
    if d_busy <= b_busy:
        return _fail(
            3,
            "[m75][P1] expected dependency any_busy_cycles > baseline "
            f"(baseline={b_busy}, dependency={d_busy})",
        )

    prefix = f"[m75:{args.label}] " if args.label else "[m75] "
    print(
