#!/usr/bin/env python3
"""Validate M108 energy/power v4 contract."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not eff_p.exists():
        return _fail(2, f"[m108][P0] missing --efficiency summary: {eff_p}")
    if not cap_p.exists():
        return _fail(2, f"[m108][P0] missing --power-cap summary: {cap_p}")

    eff = _load_tensor(eff_p)
    cap = _load_tensor(cap_p)
    if eff is None or cap is None:
        return _fail(2, "[m108][P0] invalid summary schema: missing tensor object")

    e_parts, e_total, e_per_mac = _energy_breakdown(eff)
    c_parts, c_total, c_per_mac = _energy_breakdown(cap)
    if min(e_total, c_total) <= 0.0:
        return _fail(3, f"[m108][P1] expected total energy > 0 (efficiency={e_total:.3f}, power_cap={c_total:.3f})")
    ...
    if e_mac_eff > 0.0 and c_mac_eff > 0.0 and c_mac_eff > e_mac_eff:
        return _fail(
            3,
            "[m108][P1] expected power_cap effective_mac_per_cycle <= efficiency "
            f"(efficiency={e_mac_eff:.6f}, power_cap={c_mac_eff:.6f})",
        )
    if c_per_mac < e_per_mac:
        return _fail(
            3,
            "[m108][P1] expected power_cap energy_per_mac >= efficiency "
    ...
    if c_stall < e_stall:
        return _fail(3, f"[m108][P1] expected power_cap stall_proxy >= efficiency (efficiency={e_stall}, power_cap={c_stall})")

    prefix = f"[m108:{args.label}] " if args.label else "[m108] "
    print(
