#!/usr/bin/env python3
"""Validate M26 ramulator2 + multi-MemController channel trends.

M26 intent:
- Use memHierarchy.ramulator2 backend to model DRAM timing.
- Build multiple MemControllers (channels) per PE and stripe addresses (interleave) so traffic
...
def main(argv: List[str] | None = None) -> int:
    ...
    for k, p in paths.items():
        if not p.exists():
            return _fail(2, f"[m26][P0] missing --{k} summary: {p}")

    tensors: Dict[str, Dict[str, Any]] = {}
    for k, p in paths.items():
        t = _load_tensor(p)
        if t is None:
            return _fail(2, f"[m26][P0] invalid summary schema: missing tensor object ({k})")
        tensors[k] = t

    iters = {k: _get_int(t, "tensor_program_iters_total") for k, t in tensors.items()}
    if any(v <= 0 for v in iters.values()):
        return _fail(3, f"[m26][P1] expected program_iters_total > 0: {iters}")

    reads = {k: _get_int(t, "tensor_mem_bytes_read_total") for k, t in tensors.items()}
    if any(v <= 0 for v in reads.values()):
        return _fail(3, f"[m26][P1] expected mem_bytes_read_total > 0: {reads}")
    if len(set(reads.values())) != 1:
        return _fail(3, f"[m26][P1] expected mem_bytes_read_total match: {reads}")

    dma_busy = {k: _get_int(t, "tensor_program_dma_busy_cycles_total") for k, t in tensors.items()}
    if any(v <= 0 for v in dma_busy.values()):
        return _fail(3, f"[m26][P1] expected program_dma_busy_cycles_total > 0: {dma_busy}")

    # Ensure the backend is ramulator2 (best-effort; do not fail if effective_config.json missing).
    for k, p in paths.items():
        mesh_cfg = _load_effective_mesh_cfg(p)
        if mesh_cfg and mesh_cfg.get("mem_backend") != "ramulator2":
            return _fail(3, f"[m26][P1] expected mem_backend='ramulator2' ({k}) got {mesh_cfg.get('mem_backend')!r}")
        if mesh_cfg:
            cfg = (mesh_cfg.get("mem_backend_params") or {}).get("configFile")
            if not str(cfg or "").strip():
                return _fail(3, f"[m26][P1] expected mem_backend_params.configFile present ({k})")

    if dma_busy["ch4"] >= dma_busy["ch1"]:
        return _fail(
            3,
            f"[m26][P1] expected dma_busy(4ch) < 1ch (ch1={dma_busy['ch1']} ch4={dma_busy['ch4']})",
        )
    if dma_busy["hot"] <= dma_busy["spread"]:
        return _fail(
            3,
            f"[m26][P1] expected dma_busy(hot) > spread (hot={dma_busy['hot']} spread={dma_busy['spread']})",
        )

    prefix = f"[m26:{args.label}] " if args.label else "[m26] "
