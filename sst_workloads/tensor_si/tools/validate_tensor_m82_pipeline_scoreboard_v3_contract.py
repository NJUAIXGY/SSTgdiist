#!/usr/bin/env python3
"""Validate M82 pipeline-scoreboard v3 contract."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not base_p.exists():
        return _fail(2, f"[m82][P0] missing --baseline summary: {base_p}")
    if not stress_p.exists():
        return _fail(2, f"[m82][P0] missing --stress summary: {stress_p}")

    base = _load_tensor(base_p)
    stress = _load_tensor(stress_p)
    if base is None or stress is None:
        return _fail(2, "[m82][P0] invalid summary schema: missing tensor object")
    ...
    if b_issue <= s_issue:
        return _fail(3, f"[m82][P1] expected baseline issue_width > stress (baseline={b_issue}, stress={s_issue})")
    if s_lat <= b_lat:
        return _fail(3, f"[m82][P1] expected stress pipeline_latency > baseline (baseline={b_lat}, stress={s_lat})")

    b_ops = _to_int(base.get("tensor_program_ops_total"))
    s_ops = _to_int(stress.get("tensor_program_ops_total"))
    if b_ops <= 0 or s_ops <= 0:
        return _fail(3, f"[m82][P1] expected program_ops_total > 0 (baseline={b_ops}, stress={s_ops})")
    ...
    if s_busy <= b_busy:
        return _fail(3, f"[m82][P1] expected stress any_busy_cycles > baseline (baseline={b_busy}, stress={s_busy})")
    ...
    if s_compute <= b_compute:
        return _fail(
            3,
            "[m82][P1] expected stress compute_cycles_total > baseline "
    ...
    prefix = f"[m82:{args.label}] " if args.label else "[m82] "
    print(
