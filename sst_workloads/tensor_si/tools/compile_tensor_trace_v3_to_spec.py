#!/usr/bin/env python3
"""Compile tensor trace-v3 JSON into schema-v3 tensor spec."""
...
class TraceSpecV3Error(RuntimeError):
    pass
...
def _normalize_ops(raw_ops: List[Any]) -> Tuple[List[Dict[str, Any]], int, int, int]:
    if not raw_ops:
        raise TraceSpecV3Error("trace_v3.program_ops must be a non-empty list")
    ...
    for i, op in enumerate(raw_ops):
        if not isinstance(op, dict):
            raise TraceSpecV3Error(f"trace_v3.program_ops[{i}] must be an object")
        op_type = _as_str(op.get("op_type"), "").lower()
        if op_type not in {"dma_read", "dma_write", "gemm_ub", "allreduce", "fence", "softmax", "gemm"}:
            raise TraceSpecV3Error(
                f"trace_v3.program_ops[{i}].op_type={op_type!r} invalid "
        ...
        for d in deps:
            ...
            if dep_i < 0 or dep_i >= i:
                raise TraceSpecV3Error(f"trace_v3.program_ops[{i}].deps contains invalid index={dep_i}")
        ...
        if resource not in ALLOWED_RESOURCES:
            raise TraceSpecV3Error(f"trace_v3.program_ops[{i}].resource={resource!r} invalid")
        ...
        if op_type in {"dma_read", "dma_write", "allreduce"} and _as_int(norm.get("bytes"), 0) <= 0:
            raise TraceSpecV3Error(f"trace_v3.program_ops[{i}].bytes must be > 0 for {op_type}")
...
def compile_trace_v3_to_spec(trace: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, int]]:
    ...
    tile_windows = max(0, _as_int(metadata.get("tile_windows"), 0))
    barrier_count = max(0, _as_int(metadata.get("barrier_count"), 0))
    resource_windows = max(0, _as_int(metadata.get("resource_windows"), 0))

    params: Dict[str, Any] = {
        "tensor_capability_profile": "trace_v3_bridge_v1",
        "tensor_scheduler_model": "trace_v3_compiled",
        ...
        "tensor_trace_v3_version": 3,
        "tensor_trace_v3_op_count": op_count,
        "tensor_trace_v3_dependency_edges": dep_edges,
        "tensor_trace_v3_resource_kinds": resource_kinds,
        "tensor_trace_v3_tile_windows": tile_windows,
        "tensor_trace_v3_barrier_count": barrier_count,
        "tensor_trace_v3_resource_windows": resource_windows,
    }
...
def main(argv: List[str] | None = None) -> int:
    ...
    ap.add_argument("--trace", required=True, help="input trace-v3 json")
    ...
    if not trace_p.exists():
        raise SystemExit(f"[m75][P0] trace not found: {trace_p}")

    try:
        payload = json.loads(trace_p.read_text(encoding="utf-8"))
    except Exception as exc:
        raise SystemExit(f"[m75][P0] invalid trace json: {exc}")
    if not isinstance(payload, dict):
        raise SystemExit("[m75][P0] trace json must be an object")

    try:
        spec, meta = compile_trace_v3_to_spec(payload)
    except TraceSpecV3Error as exc:
        raise SystemExit(f"[m75][P1] {exc}")

    out_p.parent.mkdir(parents=True, exist_ok=True)
    out_p.write_text(json.dumps(spec, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"[m75] compiled_spec: {out_p}")
    print(
        f"[m75] trace_v3_meta op_count={meta['op_count']} "
        f"dependency_edges={meta['dependency_edges']} resource_kinds={meta['resource_kinds']} "
        f"tile_windows={meta['tile_windows']} barrier_count={meta['barrier_count']} "
        f"resource_windows={meta['resource_windows']}"
    )
    return 0
