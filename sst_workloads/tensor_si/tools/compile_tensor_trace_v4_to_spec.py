#!/usr/bin/env python3
"""Compile tensor trace-v4 JSON into schema-v3 tensor spec."""
...
class TraceSpecV4Error(RuntimeError):
    pass
...
def _normalize_ops(raw_ops: List[Any]) -> Tuple[List[Dict[str, Any]], int, int, int]:
    if not raw_ops:
        raise TraceSpecV4Error("trace_v4.program_ops must be a non-empty list")
    ...
    for i, op in enumerate(raw_ops):
        if not isinstance(op, dict):
            raise TraceSpecV4Error(f"trace_v4.program_ops[{i}] must be an object")
        op_type = _as_str(op.get("op_type"), "").lower()
        if op_type not in {"dma_read", "dma_write", "gemm_ub", "allreduce", "fence", "softmax", "gemm"}:
            raise TraceSpecV4Error(
                f"trace_v4.program_ops[{i}].op_type={op_type!r} invalid "
        ...
        for d in deps:
            ...
            if dep_i < 0 or dep_i >= i:
                raise TraceSpecV4Error(f"trace_v4.program_ops[{i}].deps contains invalid index={dep_i}")
        ...
        if resource not in ALLOWED_RESOURCES:
            raise TraceSpecV4Error(f"trace_v4.program_ops[{i}].resource={resource!r} invalid")
        ...
        if op_type in {"dma_read", "dma_write", "allreduce"} and _as_int(norm.get("bytes"), 0) <= 0:
            raise TraceSpecV4Error(f"trace_v4.program_ops[{i}].bytes must be > 0 for {op_type}")
...
def compile_trace_v4_to_spec(trace: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, int]]:
    ...
    params: Dict[str, Any] = {
        "tensor_capability_profile": "trace_v4_bridge_v1",
        "tensor_scheduler_model": "trace_v4_compiled",
...
def main(argv: List[str] | None = None) -> int:
    ...
    ap.add_argument("--trace", required=True, help="input trace-v4 json")
    ...
    if not trace_p.exists():
        raise SystemExit(f"[m85][P0] trace not found: {trace_p}")

    try:
        payload = json.loads(trace_p.read_text(encoding="utf-8"))
    except Exception as exc:
        raise SystemExit(f"[m85][P0] invalid trace json: {exc}")
    if not isinstance(payload, dict):
        raise SystemExit("[m85][P0] trace json must be an object")

    try:
        spec, meta = compile_trace_v4_to_spec(payload)
    except TraceSpecV4Error as exc:
        raise SystemExit(f"[m85][P1] {exc}")

    out_p.parent.mkdir(parents=True, exist_ok=True)
    out_p.write_text(json.dumps(spec, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"[m85] compiled_spec: {out_p}")
    print(
        f"[m85] trace_v4_meta op_count={meta['op_count']} "
