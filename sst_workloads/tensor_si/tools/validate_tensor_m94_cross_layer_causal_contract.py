#!/usr/bin/env python3
"""Validate M94 cross-layer causal contract (memory-first vs noc-first)."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not mem_p.exists():
        return _fail(2, f"[m94][P0] missing --memory summary: {mem_p}")
    if not noc_p.exists():
        return _fail(2, f"[m94][P0] missing --noc summary: {noc_p}")

    mem = _load_ready(mem_p)
    noc = _load_ready(noc_p)
    if mem is None or noc is None:
        return _fail(2, "[m94][P0] invalid summary schema: missing npu_tpu_readiness")

    mem_dom = _dominant_layer(mem)
    noc_dom = _dominant_layer(noc)
    if mem_dom != "memory":
        return _fail(3, f"[m94][P1] expected memory scenario dominant_layer=memory (got {mem_dom!r})")
    if noc_dom not in {"memory", "noc"}:
        return _fail(3, f"[m94][P1] expected noc scenario dominant_layer in {{memory,noc}} (got {noc_dom!r})")
    ...
    if mem_pressure <= mem_noc_pressure:
        return _fail(
            3,
            "[m94][P1] expected memory scenario memory_pressure > noc_pressure "
            f"(memory={mem_pressure}, noc={mem_noc_pressure})",
        )
    if noc_pressure <= mem_noc_pressure:
        return _fail(
            3,
            "[m94][P1] expected noc scenario noc_pressure > memory-scenario noc_pressure "
    ...
    if noc_noc_share <= mem_noc_share:
        return _fail(
            3,
            "[m94][P1] expected noc layer_share increase in noc scenario "
            f"(memory_scenario_noc_share={mem_noc_share:.6f}, noc_scenario_noc_share={noc_noc_share:.6f})",
        )

    prefix = f"[m94:{args.label}] " if args.label else "[m94] "
    print(
