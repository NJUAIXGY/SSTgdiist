#!/usr/bin/env python3
"""Validate M71 readiness->counter mapping contract."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not cfg_p.exists():
        return _fail(2, f"[m71][P0] missing --config-only summary: {cfg_p}")
    if not rich_p.exists():
        return _fail(2, f"[m71][P0] missing --evidence-rich summary: {rich_p}")
    ...
    if cfg_tensor is None or cfg_ready is None or rich_tensor is None or rich_ready is None:
        return _fail(2, "[m71][P0] invalid summary schema: missing tensor/npu_tpu_readiness")
    ...
    if "readiness_mapping_v4" not in cfg_profile or "readiness_mapping_v4" not in rich_profile:
        return _fail(
            3,
            "[m71][P1] expected capability_profile contains readiness_mapping_v4 "
    ...
    if not isinstance(cfg_breakdown, dict) or not isinstance(rich_breakdown, dict):
        return _fail(3, "[m71][P1] missing capability_score_breakdown")

    for k in MAPPING_KEYS:
        if k not in cfg_breakdown or k not in rich_breakdown:
            return _fail(3, f"[m71][P1] missing readiness breakdown key: {k}")
    ...
    for k in MAPPING_KEYS:
        if int(cfg_cov.get(k, 0)) <= 0:
            return _fail(3, f"[m71][P1] config_only missing counter mapping for dimension={k}")
        if int(rich_cov.get(k, 0)) <= 0:
            return _fail(3, f"[m71][P1] evidence_rich missing counter mapping for dimension={k}")
    ...
    if rich_score <= cfg_score:
        return _fail(3, f"[m71][P1] expected evidence_rich score > config_only (config_only={cfg_score:.3f}, evidence_rich={rich_score:.3f})")
    if cfg_conf not in {"unverified", "low"}:
        return _fail(3, f"[m71][P1] expected config_only calibration_confidence in {{unverified,low}} (got {cfg_conf!r})")
    if rich_conf not in {"medium", "high"}:
        return _fail(3, f"[m71][P1] expected evidence_rich calibration_confidence in {{medium,high}} (got {rich_conf!r})")

    prefix = f"[m71:{args.label}] " if args.label else "[m71] "
