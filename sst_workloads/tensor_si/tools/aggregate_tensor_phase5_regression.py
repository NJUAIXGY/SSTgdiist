#!/usr/bin/env python3
"""Aggregate M90 phase5 regression gate results."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not results_p.exists():
        print(f"[m90][P0] missing --results-file: {results_p}", file=sys.stderr)
        return 2

    try:
        rows = _parse_results(results_p)
    except Exception as exc:
        print(f"[m90][P0] invalid results file: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"[m90][P0] write report failed: {exc}", file=sys.stderr)
        return 2
