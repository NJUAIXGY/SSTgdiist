#!/usr/bin/env python3
"""Validate M96 calibration-loop v3 profile contract and determinism."""
...
def main(argv: List[str] | None = None) -> int:
    ...
    for p, flag in ((profile_p, "--profile"), (baseline_p, "--baseline"), (target_p, "--target"), (ref_p, "--reference")):
        if not p.exists():
            return _fail(2, f"[m96][P0] missing {flag}: {p}")

    try:
        profile = _load_json(profile_p)
    except Exception as exc:
        return _fail(2, f"[m96][P0] cannot parse profile json: {exc}")
    if not isinstance(profile, dict):
        return _fail(2, "[m96][P0] profile root must be object")

    factors = profile.get("factors")
    if not isinstance(factors, dict):
        return _fail(3, "[m96][P1] missing factors object")
    for key in ("read_latency_scale", "dma_busy_scale", "suggested_mem_bandwidth_scale"):
        ...
        if not ok or val <= 0.0:
            return _fail(3, f"[m96][P1] invalid factor {key}: {factors.get(key)!r}")

    evidence = profile.get("evidence")
    if not isinstance(evidence, dict):
        return _fail(3, "[m96][P1] missing evidence object")
    status = str(evidence.get("status", "")).strip().lower()
    if status not in {"pass", "fail"}:
        return _fail(3, f"[m96][P1] invalid evidence.status={status!r}")
    ok_err, err_score = _to_float(evidence.get("error_score"))
    if not ok_err or err_score < 0.0:
        return _fail(3, f"[m96][P1] invalid evidence.error_score={evidence.get('error_score')!r}")

    conf = str(profile.get("confidence", "")).strip().lower()
    if conf not in {"low", "medium", "high"}:
        return _fail(3, f"[m96][P1] invalid confidence={conf!r}")
    if conf == "low":
        return _fail(3, "[m96][P1] expected confidence in {medium,high}")

    tag = str(profile.get("calibration_tag", "")).strip()
    if not tag:
        return _fail(3, "[m96][P1] calibration_tag must be non-empty")

    try:
        expected = build_profile(baseline=baseline_p, target=target_p, reference=ref_p, tag=tag)
    except Exception as exc:
        return _fail(3, f"[m96][P1] cannot recompute profile: {exc}")

    for key in ("read_latency_scale", "dma_busy_scale", "suggested_mem_bandwidth_scale"):
        ok_close, msg = _assert_close(key, float(factors.get(key)), float(expected["factors"][key]), tol=1e-6)
        if not ok_close:
            return _fail(3, f"[m96][P1] {msg}")
    ...
    if base_tensor is None or target_tensor is None:
        return _fail(2, "[m96][P0] invalid baseline/target summary schema")
    ...
    if target_lat <= base_lat:
        return _fail(
            3,
            "[m96][P1] expected target avg_read_latency > baseline "
            f"(baseline={base_lat:.6f}, target={target_lat:.6f})",
        )

    if int(args.expect_pass) == 1 and status != "pass":
        return _fail(3, f"[m96][P1] expected pass but evidence.status={status} error_score={err_score:.6f}")

    prefix = f"[m96:{args.label}] " if args.label else "[m96] "
    print(
