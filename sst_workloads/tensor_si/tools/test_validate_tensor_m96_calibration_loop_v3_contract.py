#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Unit tests for validate_tensor_m96_calibration_loop_v3_contract.py."""
...
SCRIPT = THIS_DIR / "validate_tensor_m96_calibration_loop_v3_contract.py"


class ValidateTensorM96CalibrationLoopV3ContractTest(unittest.TestCase):
    ...
    def test_happy_path(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            profile.write_text(
                json.dumps(build_profile(baseline=base, target=target, reference=ref, tag="m96")),
                encoding="utf-8",
            )
    ...
    def test_fail_confidence(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            target.write_text(json.dumps(self._summary(read_total=4000, read_samples=100, dma_busy=2000)), encoding="utf-8")
            ref.write_text(json.dumps(self._reference()), encoding="utf-8")
            payload = build_profile(baseline=base, target=target, reference=ref, tag="m96")
