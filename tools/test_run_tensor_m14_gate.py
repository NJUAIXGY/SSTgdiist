...
# -*- coding: utf-8 -*-
"""
CLI tests for tools/run_tensor_m14_gate.sh.
"""
...
SCRIPT = REPO_ROOT / "tools" / "run_tensor_m14_gate.sh"


class RunTensorM14GateTest(unittest.TestCase):
    def test_help(self) -> None:
        cmd = ["bash", str(SCRIPT), "--help"]
        out = subprocess.run(cmd, text=True, capture_output=True, cwd=str(REPO_ROOT))
        self.assertEqual(out.returncode, 0, msg=out.stderr)
        self.assertIn("run_tensor_m14_gate.sh", out.stdout)
