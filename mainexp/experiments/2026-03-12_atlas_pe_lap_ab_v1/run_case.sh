...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <baseline_step1_seed_only_frac003|atlas_pe_storage_only_step1_seed_only_frac003|atlas_pe_lap_step1_seed_only_frac003>" >&2
  exit 2
fi
...
case "$CASE_ID" in
  baseline_step1_seed_only_frac003)
    ...
    ;;
  atlas_pe_storage_only_step1_seed_only_frac003)
    CASE_ENV=(
      MESH_SYNAPSE_WEIGHT_MODE=gcss_valueonly_dstpe_atlas_v1
      MESH_GCSSATLAS_DIR=/home/xgy/remote/sst_dram_si/weights/gcss_valueonly_dstpe_atlas_v1_fanout256_10k
      MESH_GAS_ATLAS_PE_ENABLE=1
    )
    ;;
  atlas_pe_lap_step1_seed_only_frac003)
    CASE_ENV=(
      MESH_SYNAPSE_WEIGHT_MODE=gcss_valueonly_dstpe_atlas_v1
      MESH_GCSSATLAS_DIR=/home/xgy/remote/sst_dram_si/weights/gcss_valueonly_dstpe_atlas_lap_v1_fanout256_10k
      MESH_GAS_ATLAS_PE_ENABLE=1
    )
    ;;
  *)
