#!/usr/bin/env python3
"""Build a minimal compare.tsv snapshot for the PULSE core-level AB run."""
...
PULSE_SUM_MAX_METRICS: Tuple[str, ...] = (
...
PULSE_SUM_SUM_METRICS: Tuple[str, ...] = (
...
ALL_PULSE_METRICS: Tuple[str, ...] = PULSE_SUM_MAX_METRICS + PULSE_SUM_SUM_METRICS
...
def _read_stats_rows(path: Path) -> Iterable[Dict[str, str]]:
    if not path.exists():
        return ()
...
def _aggregate_pulse_metrics(run_dir: Path) -> Dict[str, int]:
    rows = _read_stats_rows(run_dir / "mesh_stats.csv")
    sums: Dict[str, int] = {name: 0 for name in ALL_PULSE_METRICS}
    for row in rows:
        name = (row.get("StatisticName") or "").strip()
        if name in PULSE_SUM_MAX_METRICS:
            sums[name] += _parse_int(row.get("Max.u64", "0"))
        elif name in PULSE_SUM_SUM_METRICS:
            sums[name] += _parse_int(row.get("Sum.u64", "0"))
    return sums
...
def _collect_case_metrics(run_dir: Path) -> Dict[str, Any]:
    ...
    pulse = _aggregate_pulse_metrics(run_dir)
    ...
    for metric in ALL_PULSE_METRICS:
...
def _write_compare(
    ...
    lines = [
        ...
        "# profile\tpulse_core_step1_observe_only",
        f"metric\tbaseline\t{candidate_label}\tdelta",
    ]
    ordered_metrics = MAINLINE_METRICS + ALL_PULSE_METRICS
