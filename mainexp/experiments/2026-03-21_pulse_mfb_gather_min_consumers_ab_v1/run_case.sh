...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <pulse_shared_line_actual_baseline|pulse_shared_line_actual_mfb_gather_preband_top24_lines4_budget6_min2|pulse_shared_line_actual_mfb_gather_preband_top24_lines4_budget6_min3>" >&2
  exit 2
fi
...
case "$CASE_ID" in
  pulse_shared_line_actual_baseline)
    CASE_ENV=()
    ;;
  pulse_shared_line_actual_mfb_gather_preband_top24_lines4_budget6_min2)
    CASE_ENV=(
      MESH_PULSE_MFB_GATHER_PREBAND_ENABLE=1
      MESH_PULSE_MFB_GATHER_TOP_BANDS=24
      MESH_PULSE_MFB_GATHER_LINES_PER_BAND=4
      MESH_PULSE_MFB_GATHER_MIN_CONSUMERS=2
      MESH_PULSE_MFB_GATHER_WINDOW_BUDGET=6
    )
    ;;
  pulse_shared_line_actual_mfb_gather_preband_top24_lines4_budget6_min3)
    CASE_ENV=(
      MESH_PULSE_MFB_GATHER_PREBAND_ENABLE=1
      MESH_PULSE_MFB_GATHER_TOP_BANDS=24
      MESH_PULSE_MFB_GATHER_LINES_PER_BAND=4
