...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <idx2_ingress_carry_guard_frontier_k32|idx2_ingress_carry_guard_frontier_k64>" >&2
  exit 2
fi
...
COMMON_ENV=(
  ...
  MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_MAX_INFLIGHT=8
  MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_GATHER_ONLY=1
  MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_CARRY_TO_APPLY_ENABLE=1
  MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_APPLY_MAX_INFLIGHT=1
  MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_APPLY_OUTSTANDING_RESERVE=48
...
case "$CASE_ID" in
  idx2_ingress_carry_guard_frontier_k32)
    CASE_ENV=(
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_APPLY_FRONTIER_KEEP_PENDING=32
    )
    ;;
  idx2_ingress_carry_guard_frontier_k64)
    CASE_ENV=(
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_APPLY_FRONTIER_KEEP_PENDING=64
    )
    ;;
  *)
