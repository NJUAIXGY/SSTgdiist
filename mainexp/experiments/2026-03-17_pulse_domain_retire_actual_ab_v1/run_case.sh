...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <pulse_gcss_shared_line_actual_baseline|pulse_domain_retire_actual>" >&2
  exit 2
fi
...
case "$CASE_ID" in
  pulse_gcss_shared_line_actual_baseline)
    CASE_ENV=()
    ;;
  pulse_domain_retire_actual)
    CASE_ENV=(
      MESH_PULSE_DOMAIN_RETIRE_ENABLE=1
      MESH_PULSE_DOMAIN_RETIRE_OBSERVE_ONLY=0
      MESH_PULSE_DOMAIN_RETIRE_MODE=per_post
      MESH_PULSE_DOMAIN_RETIRE_RELEASE_BUDGET=0
    )
    ;;
  *)
