...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <baseline_step1_seed_only_frac003|pulse_shared_ingress_actual_step1_seed_only_frac003>" >&2
  exit 2
fi
...
case "$CASE_ID" in
  baseline_step1_seed_only_frac003)
    ...
    ;;
  pulse_shared_ingress_actual_step1_seed_only_frac003)
    CASE_ENV=(
      MESH_PULSE_ENABLE=1
