...
class MakeSnapshotTest(unittest.TestCase):
    def test_generates_compare_tsv_with_preband_metrics(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cases_path.write_text(
                json.dumps(
                    {
                        "experiment_id": "pulse_mfb_preband_actual_test",
                        "baseline_case": "baseline",
                        "cases": [
                            {"id": "baseline", "label": "baseline", "run_dir": str(baseline_run)},
                            {"id": "mfb_preband", "label": "mfb_preband", "run_dir": str(pulse_run)},
                        ],
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(rows["pulse_mfb_head_distance_sum_total"], ["0", "120", "120"])
            self.assertEqual(rows["pulse_mfb_head_distance_samples_total"], ["0", "24", "24"])
            self.assertEqual(rows["pulse_mfb_head_distance_avg"], ["0", "5.0", "5.0"])
            self.assertEqual(rows["pulse_mfb_seed_to_first_demand_cycles_total"], ["0", "360", "360"])
            self.assertEqual(rows["pulse_mfb_seed_to_first_demand_samples_total"], ["0", "30", "30"])
            self.assertEqual(rows["pulse_mfb_seed_to_first_demand_cycles_avg"], ["0", "12.0", "12.0"])
            self.assertEqual(rows["pulse_mfb_seed_ready_before_demand_total"], ["0", "18", "18"])
            self.assertEqual(rows["pulse_mfb_seed_inflight_join_total"], ["0", "12", "12"])
