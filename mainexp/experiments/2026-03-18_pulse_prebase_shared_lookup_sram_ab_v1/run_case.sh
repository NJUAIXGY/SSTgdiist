...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <gcssplp_sram_idx_baseline|gcssplp_sram_idx_prebase_lookup>" >&2
  exit 2
fi
...
COMMON_ENV=(
  ...
  MESH_STEP_ACTIVATION_FRACTION=0.001
  ...
  MESH_SRAM_WEIGHT_L0_ENABLE=0
  MESH_SRAM_STATE_ENABLE=0
  MESH_SRAM_CALIB_JSON="$SCRIPT_DIR/sram_idx_only_calib.json"
  MESH_SRAM_CALIB_STRICT=1
  MESH_PULSE_ENABLE=1
...
case "$CASE_ID" in
  gcssplp_sram_idx_baseline)
    CASE_ENV=()
    ;;
  gcssplp_sram_idx_prebase_lookup)
    CASE_ENV=(
