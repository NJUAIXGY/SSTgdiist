...
class MakeSnapshotTest(unittest.TestCase):
    def test_generates_compare_tsv_with_frontier_metrics(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_summary(
                ...
                pulse={
                    ...
                    "pulse_ready_fanout_total": 763879,
                    "pulse_actual_gate_taken_total": 763879,
                    "pulse_frontier_windows_total": 12,
                    "pulse_frontier_lines_exported_total": 96,
                    "pulse_frontier_overlap_lines_total": 24,
                    "pulse_frontier_overlap_peer_total": 36,
                    "pulse_frontier_max_exported_per_window": 16,
                    "pulse_frontier_overlap_ratio": 0.25,
                    "pulse_frontier_avg_peer_overlap": 1.5,
                },
            )
            ...
            cases_path.write_text(
                json.dumps(
                    {
                        "experiment_id": "pulse_frontier_observe_test",
                        "baseline_case": "baseline",
                        "cases": [
                            {"id": "baseline", "label": "baseline", "run_dir": str(baseline_run)},
                            {"id": "frontier", "label": "frontier", "run_dir": str(pulse_run)},
                        ],
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(rows["pulse_frontier_windows_total"], ["0", "12", "12"])
            self.assertEqual(rows["pulse_frontier_lines_exported_total"], ["0", "96", "96"])
            self.assertEqual(rows["pulse_frontier_overlap_lines_total"], ["0", "24", "24"])
            self.assertEqual(rows["pulse_frontier_overlap_peer_total"], ["0", "36", "36"])
            self.assertEqual(rows["pulse_frontier_max_exported_per_window"], ["0", "16", "16"])
            self.assertEqual(rows["pulse_frontier_overlap_ratio"], ["0", "0.25", "0.25"])
            self.assertEqual(rows["pulse_frontier_avg_peer_overlap"], ["0", "1.5", "1.5"])
