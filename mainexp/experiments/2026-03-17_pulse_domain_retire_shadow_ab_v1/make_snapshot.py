#!/usr/bin/env python3
"""Build a minimal compare.tsv snapshot for the PULSE domain-retire shadow Stage-B run."""
...
PULSE_METRICS: Tuple[str, ...] = (
    ...
    "pulse_domain_hol_recoverable_cycles_total",
    "pulse_domain_hol_recoverable_edges_total",
    "pulse_domain_ready_domains_peak",
    "pulse_domain_committable_edges_peak",
    "pulse_domain_hol_release_ratio",
)
...
def _write_compare(
    ...
    for metric in MAINLINE_METRICS + PULSE_METRICS:
        base = baseline_metrics.get(metric, 0)
        cand = candidate_metrics.get(metric, 0)
        delta = _delta_value(base, cand)
        lines.append("\t".join([metric, _format_value(base), _format_value(cand), _format_value(delta)]))
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--cases", default=str(Path(__file__).resolve().with_name("cases.json")), help="path to cases.json")
    ap.add_argument("--out", default=str(Path(__file__).resolve().with_name("snapshot") / "compare.tsv"), help="output compare.tsv path")
    args = ap.parse_args()
