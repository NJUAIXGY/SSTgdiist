...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <pulse_shared_line_actual_baseline|pulse_shared_line_actual_mfb_gather_preband_min3_top24_lines4_budget6|pulse_shared_line_actual_mfb_gather_preband_min3_top24_lines4_budget6_barrier>" >&2
  exit 2
fi
...
case "$CASE_ID" in
  ...
  pulse_shared_line_actual_mfb_gather_preband_min3_top24_lines4_budget6)
    ...
    ;;
  pulse_shared_line_actual_mfb_gather_preband_min3_top24_lines4_budget6_barrier)
    CASE_ENV=(
      MESH_PULSE_MFB_GATHER_PREBAND_ENABLE=1
