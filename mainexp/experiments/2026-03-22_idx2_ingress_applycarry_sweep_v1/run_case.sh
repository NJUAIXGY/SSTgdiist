...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <idx2_ingress_baseline_cap8|idx2_ingress_carry_cap8|idx2_ingress_carry_cap16|idx2_ingress_carry_cap32>" >&2
  exit 2
fi
...
case "$CASE_ID" in
  idx2_ingress_baseline_cap8)
    CASE_ENV=(
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_MAX_INFLIGHT=8
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_CARRY_TO_APPLY_ENABLE=0
    )
    ;;
  idx2_ingress_carry_cap8)
    CASE_ENV=(
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_MAX_INFLIGHT=8
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_CARRY_TO_APPLY_ENABLE=1
    )
    ;;
  idx2_ingress_carry_cap16)
    CASE_ENV=(
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_MAX_INFLIGHT=16
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_CARRY_TO_APPLY_ENABLE=1
    )
    ;;
  idx2_ingress_carry_cap32)
    CASE_ENV=(
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_MAX_INFLIGHT=32
      MESH_EXPERIMENTAL_IDX2_INGRESS_PREFETCH_CARRY_TO_APPLY_ENABLE=1
    )
    ;;
  *)
