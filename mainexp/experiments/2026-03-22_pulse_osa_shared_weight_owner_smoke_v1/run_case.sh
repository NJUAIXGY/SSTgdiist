...
CASE_ID="${1:-}"
if [ -z "$CASE_ID" ]; then
  echo "usage: $0 <pulse_osa_shared_weight_owner_baseline|pulse_osa_shared_weight_owner_actual>" >&2
  exit 2
fi
...
case "$CASE_ID" in
  pulse_osa_shared_weight_owner_baseline)
    CASE_ENV=(
      MESH_PULSE_OSA_SHARED_WEIGHT_OWNER_ACTUAL_ENABLE=0
    )
    ;;
  pulse_osa_shared_weight_owner_actual)
    CASE_ENV=(
