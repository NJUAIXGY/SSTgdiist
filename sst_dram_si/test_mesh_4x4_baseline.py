...
for i in range(NODE_LIMIT):
    ...
    for core_idx in range(NUM_CORES_PER_PE):
        ...
        core_memory.addParams({
            ...
            # 保持严格模式，其余与 GAS 对齐
            "flush_after_scatter": 0,
            "strict_mode": 1,
            # 禁用 granules 导出；窗口指标关闭
            "export_granules_csv": "",
            "node_id": i,
        })
...
for i, node in enumerate(nodes):
    ...
    node.enableStatistics([
        ...
        "total_neurons_fired",
        # Step 随机激发相关统计（用于分析 Step 激发次数与规模）
        "step_activation_invocations",
        "step_activation_pre_selected",
        "step_activation_spike_attempts",
        "step_activation_spikes_injected",
        "step_activation_route_hits",
        "step_activation_route_misses",
        "step_activation_local_drops"
    ])
