...
- 地址映射：优先银行×行分组与排序；无法可靠识别或存在随机翻译时退化为行序，确保统计稳定性与正确性。DDR4（OpenRow）可自动对齐 `bank_bits/bank_shift`；HBM2 缺省 `RandomTranslation` 时自动退行至 row。
...
- 目的：在相同 NoC 轨迹与前端参数下，对比两套 DRAM 后端的端到端差异；形成“t_total 拆解”的统一口径。
- 步骤：
  1) 单 PE 生成阶段事件：
     - DDR4：`./sst sst_dram_si/test_dram_si_single_pe.py`（`ramulator2_config_file=ramulator2_ddr4_openrow.cfg`）
  ...
  2) NoC 重放（一次即可，共用）：`python3 sst_dram_si/tools/noc_trace_driver.py --granules sst_dram_si/analysis/granules.csv --bw 32GiB/s --size 4 --multicast tree --out sst_dram_si/analysis/noc_trace_summary_tree.csv`
  3) 端到端分解：
     ...
     - HBM2：同上命令但使用 HBM2 路径跑出的阶段事件 CSV，输出 `end2end_hbm2.csv`
- 提示：HBM2 若保留 `RandomTranslation`，bank_row 自动退为 row；如需 bank_row 对齐，请在 cfg 中移除该段或使用“OpenRow 变体”。
