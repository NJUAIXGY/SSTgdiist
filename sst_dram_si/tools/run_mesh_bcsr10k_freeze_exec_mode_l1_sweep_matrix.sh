...
# Matrix runner for 10k/PE + BCSR (fanout256_10k) "freeze dynamics" experiments.
#
...
# Freeze profile invariants (avoid cross-experiment stickiness).
...
echo "[mesh-bcsr10k-matrix] freeze: step_reset_mem_each_step=$MESH_STEP_RESET_MEM_EACH_STEP allow_zero_firing_long=$MESH_ALLOW_ZERO_FIRING_LONG gas_step_seq_gate=$MESH_GAS_STEP_SEQ_GATE_ENABLE"
...
PY
      ...
      while [ "$success" -lt "$MESH_SWEEP_REPEATS" ] && [ "$attempt" -lt "$attempt_max" ]; do
        ...
        if "$SCRIPT_DIR/run_mesh_bcsr10k_freeze_exec_mode_l1_sweep_with_time.sh" "$mode"; then
          success=$((success + 1))
        else
          rc=$?
